...
public class CustomRatingBar extends View {
    ...
    public CustomRatingBar(Context mContext, AttributeSet attrs) {
        super(mContext, attrs);
        init(mContext, attrs);
    }

    public CustomRatingBar(Context mContext, AttributeSet attrs, int defStyleAttr) {
    ...
    private void init(Context mContext, AttributeSet attrs){
        ...
        //初始化控件属性
        TypedArray typedArray = mContext.obtainStyledAttributes(attrs, R.styleable.CustomRatingBar);
        starNum = typedArray.getInteger(R.styleable.CustomRatingBar_starsNum, 5);
        starHeight = (int) typedArray.getDimension(R.styleable.CustomRatingBar_starHeight, 0);
        starWidth = (int) typedArray.getDimension(R.styleable.CustomRatingBar_starWidth, 0);
        starDistance = (int) typedArray.getDimension(R.styleable.CustomRatingBar_starDistance, 0);
        isClick =typedArray.getBoolean(R.styleable.CustomRatingBar_starClickable,true);
        starBackgroundBitmap = typedArray.getDrawable(R.styleable.CustomRatingBar_starBackground);
        starDrawDrawable = drawableToBitmap(typedArray.getDrawable(R.styleable.CustomRatingBar_starDrawBackground));
        typedArray.recycle();
        //初始化画笔
        mPaint = new Paint();
        //设置抗锯齿
